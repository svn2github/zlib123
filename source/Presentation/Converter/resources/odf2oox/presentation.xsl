...
<xsl:stylesheet version="1.0" 
  ...
  <!-- Template to get slide Id list = Added by lohith.ar -->
  <xsl:template name="GetSlideList">
    ...
    <xsl:if test='$first'>
      <p:sld>
        <xsl:attribute name ="r:id">
          <xsl:value-of select ="concat('sId',substring-after($first,'page'))"/>
        </xsl:attribute>
      </p:sld>
    </xsl:if>
    ...
    <xsl:if test='not($rest)'>
      <p:sld>
        <xsl:attribute name ="r:id">
          <xsl:value-of select ="concat('sId',substring-after($pages,'page'))"/>
        </xsl:attribute>
      </p:sld>
    </xsl:if>
  </xsl:template>

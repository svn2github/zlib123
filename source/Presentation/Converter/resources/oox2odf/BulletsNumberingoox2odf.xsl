...
<xsl:with-param name ="buSize" select ="$buSize"/>
			...
			<xsl:when test ="not(./a:p/a:pPr/@lvl) or ./a:p/a:pPr/@lvl='0'">
				<xsl:for-each select ="./a:p">
					...
					<xsl:if test ="not(./a:pPr/a:buNone)">
						...
						<!--end-->
            <xsl:variable name ="loop">
              <xsl:for-each select ="document(concat('ppt/slideLayouts/',$slideLayout))/p:sldLayout/p:cSld/p:spTree/p:sp/p:nvSpPr/p:nvPr/p:ph[@idx and (not(@type) or @type='body')]/
  ...
  <xsl:template name ="tmpListLevelPropFrmSM">
	...
	<xsl:template name="textFontMaster">
		...
		<xsl:if test="document(concat('ppt/slideMasters/',$slideMaster))//p:txStyles/p:bodyStyle/child::node()[name()=$elementName]/a:buFont/@typeface">
			<xsl:value-of select="document(concat('ppt/slideMasters/',$slideMaster))//p:txStyles/p:bodyStyle/child::node()[name()=$elementName]/a:buFont/@typeface"/>
		</xsl:if>
		<xsl:if test="not(document(concat('ppt/slideMasters/',$slideMaster))//p:txStyles/p:bodyStyle/child::node()[name()=$elementName]/a:buFont/@typeface)">
			<xsl:for-each select="document(concat('ppt/slideMasters/',$slideMaster))//p:txStyles/p:bodyStyle/child::node()[name()=$elementName]/a:defRPr">
			<xsl:choose>
				<xsl:when test="./a:latin">
					...
					<xsl:variable name="themeNameThis">						
						<xsl:for-each select="document(concat('ppt/slideMasters/_rels/',$slideMaster,'.rels'))//rels:Relationships/rels:Relationship">							
							<xsl:if test="./@Type = $themeName">
								<xsl:value-of select="substring-after(./@Target,'../theme/')"/>
							</xsl:if>
						</xsl:for-each>
					</xsl:variable>
					<xsl:for-each select="document(concat('ppt/theme/',$themeNameThis))//a:theme/a:themeElements/a:fontScheme">
				...
				<xsl:when test="./a:ea">
					...
					<xsl:variable name="themeNameThis">
						<xsl:for-each select="document(concat($slideMaster,'.rels'))//Relationship">
							<xsl:if test="@Type = $themeName">
								<xsl:value-of select="substring-after(./@Target,'../theme/')"/>
							</xsl:if>
						</xsl:for-each>
					</xsl:variable>
					<xsl:for-each select="document($themeNameThis)//a:fontScheme">
						<xsl:if test="$typeFace = 'mj'">
